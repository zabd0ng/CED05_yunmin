...
#define _PULSE_PER_DEGREE 10.26 // 1도 마다 변하는 pulse 값 (2399 - 553) / 180 단위: us
...
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, alpha, duty_dist, pulse_degree; // unit: mm
...
// initialize USS related variables
  ...
  pulse_degree = _PULSE_PER_DEGREE;
...
// adjust servo position according to the USS read value
  ...
  else if(dist_ema >= dist_min && dist_ema < dist_max){
    duty_dist = 553 + (dist_ema - 180)*10.26; // 서보가 거리에 비례해 움직이게 해주는 방정식
    myservo.writeMicroseconds(duty_dist);
